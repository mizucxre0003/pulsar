...
export default function Home() {
  ...
  const [groupId, setGroupId] = useState('-123456789') // Default mock group
  const [color, setColor] = useState('#ffffff')
  const [grid, setGrid] = useState({})

  useEffect(() => {
    // Extract group_id from initDataUnsafe.start_param if present
    ...
    const channel = supabase.channel('schema-db-changes')
      .on(
        ...
        { 
          ...
          schema: 'public', 
          table: 'group_data', 
          // Note: In a real app we'd filter by chat_id
        },
        (payload) => {
    ...
    <div className="p-4 pt-8 max-w-md mx-auto">
      ...
      <GlassCard className="flex flex-col items-center justify-center p-4">
        <canvas
          ...
          className="w-full aspect-square bg-dark-800 border-2 border-white/10 rounded-lg shadow-inner mb-6 touch-none cursor-crosshair"
          style={{ imageRendering: 'pixelated' }}
        />
        
        <div className="flex gap-4 p-2 bg-black/20 rounded-full backdrop-blur-md">
          {['#ffffff', '#ff3b30', '#34c759', '#007aff', '#ffd60a', '#1c1c1e'].map(c => (
            <button
              key={c}
              className={`w-8 h-8 rounded-full border-2 transition-transform ${
                color === c ? 'border-white scale-110 shadow-[0_0_15px_rgba(255,255,255,0.5)]' : 'border-transparent'
              }`}
