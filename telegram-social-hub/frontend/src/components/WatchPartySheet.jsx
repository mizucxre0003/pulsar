...
export default function WatchPartySheet({ groupId = '-123456789', onClose }) {
  ...
  useEffect(() => {
    const fetchState = async () => {
      const { data } = await supabase.from('group_data')
        ...
        .eq('chat_id', groupId).single();
      if (data?.current_video_url) setUrl(data.current_video_url);
      if (data) setPlaying(data.video_state === 'playing');
    };
    fetchState();

    const channel = supabase.channel(`watch-${groupId}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'group_data' }, (payload) => {
        const nd = payload.new;
        if (!nd || nd.chat_id?.toString() !== groupId?.toString()) return;
        if (nd.current_video_url !== undefined) setUrl(nd.current_video_url || '');
        setPlaying(nd.video_state === 'playing');
        if (playerRef.current && nd.video_timestamp) {
          if (Math.abs(playerRef.current.getCurrentTime() - nd.video_timestamp) > 2)
  ...
  return (
    <AnimatePresence>
      ...
      >
        <motion.div
          className="glass-panel rounded-t-3xl p-6 pb-10"
          initial={{ y: '100%' }}
          animate={{ y: 0 }}
          exit={{ y: '100%' }}
        ...
        >
          ...
          {/* Player */}
          <div className="aspect-video rounded-xl overflow-hidden mb-4 flex items-center justify-center"
            style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
            {url ? (
              <ReactPlayer ref={playerRef} url={url} playing={playing}
            ...
            ) : <p className="text-sm" style={{ color: 'rgba(255,255,255,0.4)' }}>Нет видео</p>}
          </div>
